...
func (s *responsesHTTPReplayStore) removeConversationLocked(rootResponseID string) error {
	...
	responses := make([]responseDepth, 0)
	for responseID, entry := range s.entries {
		...
		depth := 0
		currentID := responseID
		for currentID != "" && currentID != rootResponseID {
			parent, ok := s.entries[currentID]
			if !ok {
				break
			}
			depth++
			currentID = parent.parentResponseID
		}
		responses = append(responses, responseDepth{responseID: responseID, depth: depth})
	}
	sort.Slice(responses, func(i, j int) bool {
		return responses[i].depth > responses[j].depth
	})

	for _, response := range responses {
		entry, ok := s.entries[response.responseID]
		if !ok {
			continue
		}
		if s.stateDir != "" {
			path := filepath.Join(s.stateDir, responsesHTTPReplayNodeFileName(response.responseID))
			if err := os.Remove(path); err != nil && !os.IsNotExist(err) {
				return fmt.Errorf("remove replay node: %w", err)
			}
		}
		s.totalBytes -= entry.storedBytes
		delete(s.entries, response.responseID)
	}
	if s.totalBytes < 0 {
		s.totalBytes = 0
	}
